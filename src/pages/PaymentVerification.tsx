...
// feat: add payments table with verify actions and Add Payment dialog
...
// Add Payment Dialog (placed in ModuleLayout `actions` prop):
  ...
  <DialogContent className="bg-card border-white/10">
    ...
    <div className="space-y-4">
      <div>
        ...
        <Input id="user_name" value={formData.user_name}
          onChange={(e) => setFormData({ ...formData, user_name: e.target.value })}
          className="bg-black/50 border-white/10" required />
      </div>
      <div>
        <Label htmlFor="user_email">User Email (Optional)</Label>
        <Input id="user_email" type="email" value={formData.user_email}
          onChange={(e) => setFormData({ ...formData, user_email: e.target.value })}
          className="bg-black/50 border-white/10" />
      </div>
      <div>
        <Label htmlFor="transaction_id">Transaction ID</Label>
        <Input id="transaction_id" value={formData.transaction_id}
          onChange={(e) => setFormData({ ...formData, transaction_id: e.target.value })}
          className="bg-black/50 border-white/10" required />
      </div>
      <div>
        <Label htmlFor="amount">Amount (₹)</Label>
        <Input id="amount" type="number" value={formData.amount}
          onChange={(e) => setFormData({ ...formData, amount: e.target.value })}
          className="bg-black/50 border-white/10" />
      </div>
      <div className="flex justify-end space-x-2">
        <Button variant="outline" onClick={() => setDialogOpen(false)}>Cancel</Button>
        <Button onClick={handleAddPayment} className="gradient-primary">Add Payment</Button>
      </div>
    </div>
  </DialogContent>
</Dialog>
              ...
              <TableCell>
                <div className="flex space-x-2">
                  <Button variant="ghost" size="icon"
                    ...
                    disabled={payment.payment_received} title="Verify Payment">
                    <Check className="w-4 h-4 text-green-400" />
                  </Button>
                  <Button variant="ghost" size="icon"
                    onClick={() => handleVerifyPayment(payment.id, false)}
                    disabled={!payment.payment_received} title="Unverify Payment">
